...
export const Cards = ({ CardAns }) => {
  ...
  return (
    <div className="w-full h-full flex flex-col">
        <div className="flex-1 overflow-y-auto max-h-[calc(100vh-390px)]  scrollbar-none">
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 w-full ">
            {cards.map((card, index) => (
              <div
                key={index}
                onClick={() => getAns(card.question)}
                className="relative p-[16px] flex flex-col justify-between md:w-[195px] md:h-[195px] w-[165px] h-[165px] rounded-xl cursor-pointer overflow-hidden bg-[#181818] border border-transparent transition-all duration-300 ease-in-out hover:bg-gradient-to-br hover:from-[#181818] hover:to-[#D480DC] hover:via-[#738CD3] border-solid"
                style={{
                  border: "0.5px solid",
                  borderImageSource: "linear-gradient(133.66deg, #888888 -10.53%, #171717 100%)",
                  borderImageSlice: 1,
                }}
              >
                <div
                ...
                />
                <div className="absolute inset-0 p-4 flex flex-col justify-between overflow-hidden rounded-xl">
                  <div className="text-white text-[16px] md:text-[18px] absolute top-16">
                    {card.question}
                  </div>
                  <div className="flex justify-end">
                    <div className="rounded-xl bg-opacity-20 w-[32px] h-[32px] absolute top-4 left-4 flex items-center justify-center hover:bg-[#FFFFFF0F] transition-all duration-300 ease-in-out">
                      {getIcon(card.icon)}
                    </div>
                  </div>
                </div>
...
const EducationIcon = () => (
  <svg width="25" height="18" viewBox="0 0 20 20" fill="none" xmlns="http://www.w3.org/2000/svg">
    <path 
